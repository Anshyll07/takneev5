...
def get_product_details(product_id):
    ...
    try:
        with get_db_cursor() as cursor:
            cursor.execute("""
                SELECT a.*, u.name as artist_name, a.time_spent
                FROM artworks a
                JOIN users u ON a.user_email = u.email
                WHERE a.id = %s
            """, (product_id,))
...
def upload_artwork():
    ...
    try:
        ...
        if not all([corrected_title, corrected_category, material, description, time_spent]):
        ...
        with get_db_cursor(commit=True) as cursor:
            ...
            cursor.execute("""
                ...
                (id, user_email, title, category, material, description, price, files, created_at, status, views, likes, feedback, time_spent)
