...
document.getElementById('generateAllBtn')?.addEventListener('click', async () => {
    ...
    generateAllBtn.innerHTML = '<div class="tool-content"><span class="tool-name">Generating...</span></div><span class="tool-arrow">→</span>';

    try {
        const response = await fetchWithXHR('/api/generate-all-ai-features', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            ...
            if (results.price && !results.price.startsWith('Error')) {
                document.getElementById('price').value = results.price.replace(/[^0-9.]/g, '');
            }
            if (results.description && !results.description.startsWith('Error')) {
                document.getElementById('description').value = results.description;
            }
            if (results.image && !results.image.startsWith('Error')) {
                const previewContainer = document.getElementById('enhancement-preview-container');
                previewContainer.innerHTML = `
                    <h4>Enhanced Preview</h4>
                    <img id="enhanced-preview-image" src="${results.image}" alt="Enhanced Preview">
                    <button id="use-enhanced-image-btn" class="btn-primary">Use Preview Image</button>
                `;
                document.getElementById('use-enhanced-image-btn')?.addEventListener('click', useEnhancedImage);
                previewContainer.style.display = 'block';
            ...
            alert('Failed to generate all features: ' + data.message);
        }
    } catch (error) {
        console.error('Error generating all features:', error);
        alert('An error occurred while generating all features.');
    } finally {
        generateAllBtn.disabled = false;
        generateAllBtn.innerHTML = '<div class="tool-content"><span class="tool-name">Generate All</span><span class="tool-desc">Run all AI tools at once</span></div><span class="tool-arrow">→</span>';
    }
});